...
Опросы. Раздел: оформление. Нажатие на иконку луны. Проверить, что тело карточки - примера стало темным.
Опросы. Раздел: оформление. Нажатие на иконку солнца. Проверить, что тело карточки - примера стало светлым.

Опросы. Раздел: оформление. Нажатие на иконку компьютера. Проверить, что тело карточки - примера стало размера формата пк.
...
Опросы. Раздел: вопросы. Добавить вопрос типа ```Один из списка```. Заполнение поля ```Вопрос```. Добавления 2 вариантов ответа. Добавление шаблонного варианта ответа - ```Другое (свой ответ)```.
...
Опросы. Раздел: вопросы. Добавление второго вопроса. Изменить тип вопроса на ```Шкала```. Установить диапазон от 1 до 10. Заполнить поле ```Вопрос```. Изменить названия крайних значений шкалы.
...
Опросы. Раздел: вопросы. Нажать на копку ```Добавить стоп экран```. Проверить что появилась часть формы с полями: ```Завершить опрос, если ответ на```, ```содержит любой из```, ```Заголовок экрана```, ```Описание```. Нажать иконку ```Удалить Стоп-экран```. Проверить, что соответствующая часть формы исчезла.
...
Опросы. Раздел: вопросы. Нажать на копку ```Назад```. Проверить что отобразился раздел ```Оформление```, поля заполнены значениями, которые были введены ранее. Нажать на копку ```Вопросы```. Проверить что отобразился раздел ```Вопросы```, поля заполнены значениями, которые были введены ранее.
...
Опросы. Нажать на линк ```Архивировать```. Проверить что появилось модальное окно ```Архивировать опрос?```. Нажать кнопку ```Отменить```. Проверить, что опрос не удалился.
...
Опросы. Сменить отображаемое с ```Активные``` на ```В архиве```. Найди недавно удаленный опорос, нажать для него на линк ```Восстановить```. Проверить что появилось модальное окно ```Восстановить опрос?```. Нажать кнопку ```Отменить```. Проверить, что опрос не восстановился.
...
Опросы. Нажать на линк ```Редактировать```. Проверить что открывается форма редактирования. Нажать на иконку крестика. Проверить, что форма редактирования закрылась.
...
Опросы. Нажать на иконку троеточия. Проверить, что появилась форма выбора действия. Выбрать действие - ```Посмотреть тестовый опрос```. Проверить, что открылось окно с тестовым опросом. Пройти его. Проверить, что значение в поле ```Количество анкет``` увеличилось на 1.
